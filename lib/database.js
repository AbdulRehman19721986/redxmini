...
const connectdb = async () => {
    ...
    if (!config.MONGODB_URI) {
        console.warn('⚠️ MONGODB_URI is not configured; using local fallback behavior.');
        return false;
    }
    ...
    connectionPromise = (async () => {
        try {
            ...
            // Never let a request sit in Mongoose's operation buffer while
            // Atlas is unreachable. Every public function below checks the
            // connection state before querying.
...
async function getUserConfigFromMongoDB(number) {
    try {
        if (!isDatabaseReady()) return { ANTIDELETE: 'true' };
        const cleanNumber = number.replace(/[^0-9]/g, '');
...
async function verifyOTPFromMongoDB(number, otp) {
    try {
        if (!isDatabaseReady()) return { valid: false, error: 'Database unavailable' };
        const cleanNumber = number.replace(/[^0-9]/g, '');
