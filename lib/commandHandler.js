...
class CommandHandler {
  ...
  registerCommand(plugin) {
    // Older bundled plugins used registerCommand(name, definition). Accept
    // that shape for compatibility while keeping the object form canonical.
    if (typeof plugin === 'string' && arguments[1] && typeof arguments[1] === 'object') {
      plugin = { ...arguments[1], command: plugin };
    }
